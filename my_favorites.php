...
if(isset($_POST['dlte'])){
    foreach($_POST['dlte'] as $id){
        $del = mysqli_query($mysql, "DELETE FROM favorites WHERE favoriteID = $id");
    }
    header("Location: /my_favorites");
}
?>
<!DOCTYPE html>
<html lang="en">
...
<body>
    ...
    <div id="app">
        ...
        <div id="container">
            <div id="guide">
                <div class="guide_container">
                    <div>
                        ...
                        <div class="guide-item global--link" data-link="/my_favorites">
                            <span>
                                Избранное
                            </span>
                        </div>
            ...
            <form method="POST" action="/my_favorites" id="vid_manager">
                <div class="dashboard_s">
                    <h2 id="vm_h">
                        Избранное
                        <div id="vm_c">
                            <?php
                                echo $global->count('int', 'favorites', 'favoriteTo', $userID);
                            ?>
                        </div>
                    </h2>
                </div>
                <div>
                    ...
                    <?php
                    $sort = $_POST['sort'];
                    $wh = mysqli_query($mysql, "SELECT * FROM favorites LEFT JOIN videos ON videoID = favoriteVideoID LEFT JOIN users ON userID = videoBy WHERE favoriteTo = $userID AND ban = '' ORDER BY favoriteID $sort");
                    if(mysqli_num_rows(mysqli_query($mysql, "SELECT * FROM favorites LEFT JOIN videos ON videoID = favoriteVideoID LEFT JOIN users ON userID = videoBy WHERE favoriteTo = $userID AND ban != ''")) != 0){
                    ...
                    <div class="related-video">
                        <div id="rm_rl">
                            <input type="checkbox" class="checkbox" id="dltevid" name="dlte[]" value="<?php echo $video['favoriteID'] ?>">
                        </div>
