...
if(isset($_POST['dlte'])){
    foreach($_POST['dlte'] as $id){
        $del = mysqli_query($mysql, "DELETE FROM history WHERE historyID = $id");
    }
    header("Location: /my_videos_history");
}
?>
<!DOCTYPE html>
<html lang="en">
            ...
            <div id="guide">
                <div class="guide_container">
                    <div>
                        ...
                        <div class="guide-item global--link" data-link="/my_videos_history">
                            <span>
                                История просмотра
                            </span>
                        </div>
            ...
            <form method="POST" id="vid_manager">
                <div class="dashboard_s">
                    <h2 id="vm_h">
                        ...
                        <div id="vm_c">
                            <?php
                                echo $global->count('int', 'history', 'historyTo', $userID);
                            ?>
                        </div>
                    </h2>
                </div>
                <div>
                    ...
                    <?php
                    $sort = $_POST['sort'];
                    $vp = mysqli_query($mysql, "SELECT * FROM history LEFT JOIN videos ON videoID = historyVideoID LEFT JOIN users ON userID = videoBy WHERE historyTo = $userID ORDER BY historyID $sort");
                    while($video = mysqli_fetch_assoc($vp)){
                    ?>
                    <div class="related-video">
                        <div id="rm_rl">
                            <input type="checkbox" class="checkbox" id="dltevid" name="dlte[]" value="<?php echo $video['historyID'] ?>">
                        </div>
